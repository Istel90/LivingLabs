...
$schemaFile = Join-Path $PSScriptRoot 'prepare-hev-grid.sql'
...
if ($LASTEXITCODE -ne 0) { throw 'Unable to prepare HEV PostGIS schema.' }
& $psql -h $HostName -p $Port -U $User -d $Database -P pager=off -c @"
SELECT d.dataset_key,
       (SELECT count(*) FROM analysis.grid_cells_100m) AS grid_cells,
       (SELECT count(*) FROM analysis.hev_values_100m WHERE version_id = d.version_id) AS value_cells,
       (SELECT count(*) FROM analysis.region_indicator_stats WHERE version_id = d.version_id) AS region_indicators
