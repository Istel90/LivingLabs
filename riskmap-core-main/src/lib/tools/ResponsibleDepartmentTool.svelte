    ...
    function onMapClick(event) {
        ...
        if (project.geometryType === 'point') {
            const layer = project.item === '가로수'
                ...
                : L.circleMarker(event.latlng, { radius: 7, color: '#fff', weight: 2, fillColor: project.color, fillOpacity: 1 }).addTo(projectLayers);
            addFeature(project, { coordinates: [[event.latlng.lat, event.latlng.lng]], measure: 1, layer });
            return;
        }
...
{#if showDesigner}
    <div class="modal-backdrop">
        <form class="modal" onsubmit={(event) => { event.preventDefault(); completeDesign(); }}>
            <h2>적응 사업 디자인</h2>
