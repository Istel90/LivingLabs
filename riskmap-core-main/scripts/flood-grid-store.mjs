...
function assertLookup(regionCode, indicator) {
    ...
    if (!INDICATOR_COLUMNS[indicator])
        throw new Error('indicator must be FH01, FH02, FH03, FE01, FE02, or FE03');
}
    ...
    async function ready() {
        try {
            const result = await pool.query(
                `SELECT to_regclass('analysis.flood_values_100m') IS NOT NULL AS ready`
            );
