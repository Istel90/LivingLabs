...
일상적인 개발환경 이전에는 전국 SHP 원본을 다시 옮길 필요가 없다. 다음 백업 파일 하나를 외장 드라이브나 별도 대용량 저장소로 복사한다.

```text
D:\90_Data\VWORLD\transfer_to_other_pc\vworld_cadastral_2026-08-08.dump
```

새 PC에 PostgreSQL 17과 PostGIS 3.6을 준비하고 로컬 서버를 시작한 뒤 복원한다. 같은 이름의 데이터베이스가 이미 있으면 스크립트가 중단된다. 기존 DB를 의도적으로 교체할 때만 `-Force`를 사용한다.

```powershell
powershell.exe -NoProfile -ExecutionPolicy Bypass -File .\scripts\restore-vworld-postgis.ps1
```

복원이 끝나면 `status-vworld-postgis.ps1`과 `/api/cadastre/health`로 39,870,653건과 API 연결 상태를 확인한다.
